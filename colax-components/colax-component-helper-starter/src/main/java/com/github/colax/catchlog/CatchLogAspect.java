...
import com.github.colax.model.exception.BizException;
import com.github.colax.model.exception.SysException;
...
public class CatchLogAspect {
     ...
     */
    @Pointcut("@within(com.github.colax.catchlog.CatchAndLog) && execution(public * *(..))")
    ...
    private Object handleException(ProceedingJoinPoint joinPoint, Throwable e) {
        ...
        if (e instanceof BizException) {
            ...
            return ResponseHandlerFactory.get().handle(returnType,
                ((BizException) e).getErrorCode(), e.getMessage());
        } else if (e instanceof SysException) {
            log.error("SYS EXCEPTION: {}",e.getMessage(), e);
          return ResponseHandlerFactory.get().handle(returnType,
              ((SysException) e).getErrorCode(), e.getMessage());
        }
